...
class NewsletterWidget extends Widget
{
    ...
    public function run() {
        ...
        $formStart = '<form id="trivial-form" action="' . $this->content['action'] . '" class="">';
        $label = '<label style="display: block;" for="' . $this->content['label_for'] . '" class="control-label">' . $this->content['label_val'] . '</label>';
        $input = '<input id="' . $this->content['input_id'] . '" type="' . $this->content['input_name'] . '" name="' . $this->content['input_type'] . '"/>';
